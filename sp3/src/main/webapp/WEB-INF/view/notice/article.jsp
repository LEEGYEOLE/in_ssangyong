...
<div class="body-container" style="width: 700px;">
	<div class="body-title">
		<h3>
			<span style="font-family: Webdings">2</span> 공지사항
		</h3>
	</div>

	<div>
		<table
			style="width: 100%; margin-top: 20px; border-spacing: 0px; border-collapse: collapse;">
			<tr height="35"
			...
			<c:if test="${empty files }">
				<tr height="35" style="border-bottom: 1px solid #cccccc;">
					<td colspan="2" align="left" style="padding-left: 5px;">첨부 :
						첨부된 파일이 없습니다.</td>
				</tr>
			</c:if>
			<c:forEach var="file" items="${files}">
				<tr height="35" style="border-bottom: 1px solid #cccccc;">
					<td colspan="2" align="left" style="padding-left: 5px;">첨부 : <a
						href="<%=cp%>/notice/download?fileNum=${file.fileNum}">${file.originalFilename}</a>
			...
			<tr height="35" style="border-bottom: 1px solid #cccccc;">
				<td colspan="2" align="left" style="padding-left: 5px;">이전글 : <c:if
						...
						<a href="<%=cp%>/notice/article?${query}&num=${preReadDto.num}">
							${preReadDto.subject}</a>
					</c:if> <c:if test="${empty preReadDto}">
			...
			<tr height="35" style="border-bottom: 1px solid #cccccc;">
				<td colspan="2" align="left" style="padding-left: 5px;">다음글 : <c:if
						...
						<a href="<%=cp%>/notice/article?${query}&num=${nextReadDto.num}">
							${nextReadDto.subject}</a>
					</c:if> <c:if test="${empty nextReadDto}">
			...
			<tr height="45">
				<td width="300" align="left">
					<button type="button" class="btn"
						onclick="updateNotice();" ${sessionScope.member.userId!="admin"?"style='pointer-events:none;'":""}>수정</button>
					<button type="button" class="btn" onclick="deleteNotice();" ${sessionScope.member.userId!="admin"?"style='pointer-events:none;'":""}>삭제</button>
				</td>

				<td align="right">
					<button type="button" class="btn"
						onclick="javascript:location.href='<%=cp%>/notice/list?${query}';">리스트</button>
				</td>
			</tr>
		</table>
	</div>
