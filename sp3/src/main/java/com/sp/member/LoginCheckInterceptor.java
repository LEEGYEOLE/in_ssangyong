...
/*
  jsp의 필터랑 같은 개념 , jsp서블릿에서 넘겨주기 전에 전작업 후작업을 해주는것...
...
public class LoginCheckInterceptor extends HandlerInterceptorAdapter {
	...
	/*
	 * 클라이언트 요청이 컨트롤러에 도착하기 전에 호출 false를 리턴하면 HandlerInterceptor 또는 컨트롤러를 실행하지 않고 요청
	 * 종료
	 */
	@Override
		...
		try {
			...
			if(info==null) {
				...
				if(isAjaxRequest(request)) {
					//ajax인 경우
					response.sendError(403);//200성공. 403에러
				}else {
					//ajax 아닌경우
					if(uri.indexOf(cp)==0) {
	...
	/*
	 * 클라이언트 요청을 처리한 뒤, 즉 뷰를 통해 클라이언트에 응답을 전송한 뒤에 실행 컨트롤러 처리 중 또는 뷰를 생성하는 과정에 예외가
	 * 발생해도 실행
	 */
	@Override
